...
This is the web server for the True Walk application, developed as part of the CPE 350/450 course at Cal Poly San Luis Obispo (2025-26) in collaboration with Carl Sloan. The web server serves two main purposes, handling API requests and serving the static files for frontend web dashboard. All data is stored in a MongoDB Atlas database, ensuring scalability and reliability. The frontend application is a single-page application (SPA) built with Cal Poly's Mustang framework and served statically from the server. Documentation for the Mustang framework can be found [here](https://github.com/kubiak-calpoly/csc-437-examples/blob/main/packages/mustang/docs/index.md).

The backend API server is responsible for handling all data ingestion, storage, and retrieval for the system. It exposes RESTful endpoints that receive pressure and activity data from the mobile application, which originates from the ESP32-based walker handles via BLE. The server processes incoming data, ensures proper formatting and validation, and stores it in a cloud-hosted database (MongoDB). It also provides endpoints for querying historical data, enabling efficient access to user metrics such as pressure trends, step counts, and rehabilitation progress.

The front-end web dashboard provides a user-friendly interface for visualizing rehabilitation data collected by the system. It retrieves data from the backend API and presents it through intuitive graphs and metrics, allowing users and clinicians to monitor progress over time. Key features include visualization of bilateral pressure distribution, activity levels, and trend analysis to assess improvements in mobility and reduced walker reliance. The dashboard is designed with clarity and accessibility in mind, prioritizing simple navigation and readable data displays, and can be extended to include more advanced analytics, customizable views, or clinician-focused reporting tools.
